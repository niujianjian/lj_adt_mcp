...
import { session_types, CreatableTypeIds } from 'abap-adt-api';
  ...
  private async handleRun(args: any): Promise<any> {
    ...
    try {
      ...
      if (args.interfaceMethod) {
        ...
        try {
          const ifSource = await this.withSession(() =>
            this.adtclient.getObjectSource('/sap/bc/adt/oo/interfaces/if_oo_adt_classrun/source/main')
          ) as string;
          methodName = ifSource.toLowerCase().includes('method.*main') ? 'main' : 'run';
        } catch (_) {
      ...
      try {
        // Use proper NewObjectOptions format for createObject
        const classType: CreatableTypeIds = 'CLAS/OC';
        const createOptions = {
          name: className,
          objtype: classType,
          description: 'Temporary ADT runner class',
          parentName: '$TMP',
          parentPath: '/sap/bc/adt/packages/%24tmp',
          responsible: ((this.adtclient as any).username || 'UNKNOWN').toUpperCase()
        };
        await this.withSession(() => this.adtclient.createObject(createOptions));
      } catch (createErr: any) {
        ...
        if (msg.includes('already exists') || msg.includes('exists already')) {
          ...
          if (deleteIt) {
            try {
              const delLock = await this.withSession(() => this.adtclient.lock(classUrl));
              await this.withSession(() => this.adtclient.deleteObject(classUrl, delLock.LOCK_HANDLE));
            } catch (_) {}
            // Retry creation
            const classType: CreatableTypeIds = 'CLAS/OC';
            const createOptions = {
      ...
      const lockResult = await this.withSession(() => this.adtclient.lock(classUrl));
      lockHandle = lockResult.LOCK_HANDLE || '';

      const source = this.buildClassSource(className, args.methodBody, methodName);
      await this.withSession(() => this.adtclient.setObjectSource(sourceUrl, source, lockHandle));

      await this.withSession(() => this.adtclient.unLock(classUrl, lockHandle));
      lockHandle = '';

      // Activate — if it fails because the method name is wrong for this release, surface a clear hint
      const activationResult = await this.withSession(() => this.adtclient.activate(className, classUrl));
      ...
      // Use stateless session for classrun to avoid activation commit issues.
      const h = (this.adtclient as any).h;
      let output: any;
      try {
        // Use POST with appropriate headers for classrun
        const response = await this.withSession(() =>
          h.request(`/sap/bc/adt/oo/classrun/${className.toUpperCase()}`, {
            method: 'POST',
            headers: { 
              Accept: 'text/plain',
        ...
        const respObj = response as { body?: string; data?: string };
        output = respObj.body ?? respObj.data ?? '';
      } catch (runError: any) {
        ...
        // Special handling for 403 - might need re-authentication
        if (status === 403) {
          throw new Error(
            `classrun returned 403 Forbidden. Possible causes: ` +
            `(1) Missing authorization for ADT classrun service, ` +
            `(2) Session expired. Try calling login() to refresh credentials.${bodyStr ? ` SAP response: ${bodyStr}` : ''}`
          );
        }
        ...
        if (status === 500 || msg.includes('500')) {
          ...
          throw new Error(
            ...
            `(3) Wrong interface method used (try interfaceMethod="${methodName === 'run' ? 'main' : 'run'}").${detail}`
          );
        }
        const detail = bodyStr ? ` SAP response: ${bodyStr}` : '';
      ...
      if (lockHandle && lockHandle.length > 0) {
        try { await this.withSession(() => this.adtclient.unLock(classUrl, lockHandle)); } catch (_) {}
      }
      ...
      if (classCreated && !args.keepClass) {
        try {
          const deleteLock = await this.withSession(() => this.adtclient.lock(classUrl));
          await this.withSession(() => this.adtclient.deleteObject(classUrl, deleteLock.LOCK_HANDLE));
        } catch (_) {
